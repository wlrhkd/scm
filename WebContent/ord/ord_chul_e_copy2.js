...
function onSave(e) {
    if (jfUpdateChk(e.target.id)) {
        ...
        $('#tbl_wrap tr:gt(0)').each(function (row, tr) {
            ...
            const vSelectboxes = $("#row_select option:selected").val();
        ...
        if (bUpdateStat) {
            $.ajax({
                type: "POST",
                url: "../ord_chul_e",
                data: { ActGubun: "I", JsonData: JSON.stringify(vRowData)},
...
function pgCallBackSync(response) {
...
$(document).on('click', 'input:radio[name=gubun]', function () {
