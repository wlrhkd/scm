...
// 콤보박스
//----------------------------------------------------------------
    cfGetHeadCombo("ord_junsu_q", "saupj", "sel_saupj"); //사업장 "sel_saupj" : id값     
    setInitValue();
    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("scm/ord_junsu_q", "message" ,"110");
                return false;
            }
...
function pgLoadData(page) {
    ...
    if (vCvcod == "" || vCvcod == null) {
        cfGetMessage("scm/ord_junsu_q", "message", "250");
        return false;
    }
    ...
    let oResponse = cfAjaxSync("POST", "scm/ord_junsu_q", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/ord/ord_junsu_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
        ...
        for (i = 0; i < oResult.length; i++) {
            ...
            gvOrgData.push({
				...
				"BALDATE": aItem["BALDATE"],//발주일자                
				"GUDAT": aItem["GUDAT"],    //납기요구일자                     
                "NADATE": aItem["NADATE"],	//납기일자  
                "BALQTY": aItem["BALQTY"],  //발주량
                "RCQTY": aItem["RCQTY"],    //납품량
                "GABQTY": aItem["GABQTY"],  //미납량
                "GABDAY": aItem["GABDAY"]   //초과일수
            });
        }
        $("#tbl_wrap").show();
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/ord_junsu_q", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/ord/ord_junsu_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    switch(vEventId) {
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                cfAjaxAsync("GET","scm/ord_junsu_q",oData,"txt_cvcod");                    
            } else {
					cfSetValue("txt_cvnas","");
			}
        }
		...
		case "txt_itnbr":
                    ...
                    cfAjaxAsync("GET","scm/ord_junsu_q",iData,"txt_itnbr");         
                } else {
					cfSetValue("txt_itdsc","");
				}
            }
...
function onExcel() {	
	const vToday = cfGetToday();
	fnExcelReport('tbl_wrap_excel', '납입준수현황-'+ cfStringFormat(vToday, "D1"));
}
