...
// 콤보박스
//----------------------------------------------------------------
    cfGetHeadCombo("fa_jibul_q", "saupj", "sel_saupj");
    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("scm/fa_jibul_q", "message" ,"50");
                return false;
            }
...
function pgLoadData(page) {
    ...
    let oResponse = cfAjaxSync("POST", "scm/fa_jibul_q", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/fa/fa_jibul_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
        ...
        for (i = 0; i < oResult.length; i++) {
            ...
            gvOrgData.push({
                ...
                "GYUL_DATE": aItem["GYUL_DATE"],   	// 지불일자 
                "TOTAMT": aItem["TOTAMT"],    		// 지불금액                      
				"CASHAMT": aItem["CASHAMT"],		// 현금       
				"BILLAMT": aItem["BILLAMT"],    	// 어음                     
                "BIN": aItem["BIN"]					// 비고  
            });
        }
        $("#tbl_wrap").show();
        ...
        cfGetMessage("scm/fa_jibul_q", "message", "300");
        ...
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                if (vCvcod != "") {
                    ...
                    cfAjaxAsync("GET","scm/fa_jibul_q",oData,"txt_cvcod");                    
                }
            }
            break;
    }
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/fa_jibul_q", sData, "startSelectE");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/fa/fa_jibul_q.txt",null,"table"); //테이블 정보 읽어오기
...
function cfGetAuth() {
	...
	cfAjaxSync("POST", "scm/fa_jibul_q", aData, "getAuth");
}