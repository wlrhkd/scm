	...
	switch (name) {
		case "txt_itnbr":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("scm/pdt_week_q", "message" ,"110");
...
//    }
    ...
    let oResponse = cfAjaxSync("POST", "scm/pdt_week_q", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/pdt/pdt_week_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
		...
		vHeader = vHeader.replace("D2", cfDateCalc(sYymd, 1));
		vHeader = vHeader.replace("D3", cfDateCalc(sYymd, 2));
		vHeader = vHeader.replace("D4", cfDateCalc(sYymd, 3));
		vHeader = vHeader.replace("D5", cfDateCalc(sYymd, 4));
		vHeader = vHeader.replace("D6", cfDateCalc(sYymd, 5));
		vHeader = vHeader.replace("D7", cfDateCalc(sYymd, 6));
		vListHTML = vHeader;
		...
		//format
            for (let i = 0; i < vBody.rows.length; i++) {
				...
				// D13
                vBody.rows[i].cells[17].innerText = cfNumberFormat(vBody.rows[i].cells[17].innerText);
				// D14
                vBody.rows[i].cells[18].innerText = cfNumberFormat(vBody.rows[i].cells[18].innerText);
            }
        //페이징
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/pdt_week_q", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/pdt/pdt_week_q.txt",null,"table"); //테이블 정보 읽어오기
        ...
        case "txt_itnbr":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                if (vItnbr != "") {
                    ...
                    cfAjaxAsync("GET","scm/pdt_week_q",oData,"txt_itnbr");                    
                }
            }
            break;
	  }
...
// 5.사용자 정의 함수
 $('#txt_fromDate').datepicker({
            ...
            onClose: function( selectedDate ) {    
                ...
                $("#txt_fromDate").datepicker( "option", "minDate", selectedDate );
            }                
        });
//품번 모달 선택값 설정 후 종료
