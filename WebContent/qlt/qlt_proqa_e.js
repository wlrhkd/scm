...
// 1.초기화면 설정
//----------------------------------------------------------------------------------------------------------------------------
    cfBtnHide("insert","save","modify","help","add","delete","cancel","print");
    // 날짜
...
// 콤보박스
//----------------------------------------------------------------
    cfGetHeadCombo("qlt_proqa_e", "saupj", "sel_saupj"); //사업장
    setInitValue();
//	pgLoadData(1);
	// 관리자/사용자
	cfGetAuth();
    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("qlt_proqa_e", "message" ,"50");
                return false;
            }
...
function pgLoadData(page) {
    ...
    let oResponse = cfAjaxSync("POST", "qlt_proqa_e", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","qlt/qlt_proqa_e.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
        ...
        for (i = 0; i < oResult.length; i++) {
            ...
            gvOrgData.push({
                ...
                "ITNBR": aItem["ITNBR"],     
                "ITDSC": aItem["ITDSC"],       
                "IOFAQTY": aItem["IOFAQTY"],         
				"IOFATXT": aItem["IOFATXT"],    
				"JOCHYDAT": aItem["JOCHYDAT"],	    
				"BIN": aItem["BIN"],	    
				"DAE": aItem["DAE"],	    
				"SUBMIT_YN": aItem["SUBMIT_YN"],    
				"JOCHWDAT": aItem["JOCHWDAT"],	    
				"SAUPJ": aItem["SAUPJ"]	    
            });
        }
        $("#tbl_wrap").show();
		...
		// format
            for (let i = 0; i < vBody.rows.length; i++) {
                vBody.rows[i].cells[3].innerText = cfStringFormat(vBody.rows[i].cells[3].innerText,"D2");   //발생일자
                vBody.rows[i].cells[6].innerText = cfStringFormat(vBody.rows[i].cells[6].innerText,"N");    //발생수량
				vBody.rows[i].cells[8].innerText = cfStringFormat(vBody.rows[i].cells[8].innerText,"D2");   //개선요구일
				vBody.rows[i].cells[12].innerText = cfStringFormat(vBody.rows[i].cells[12].innerText,"D2"); //개선완료일
            }
        //페이징
        const oPageNav = cfDrawPageNav(nTotalRecords, nRecordPerPage, nPageNo);
        $("#oPaginate").html(oPageNav);
    } else {
        cfGetMessage("qlt_proqa_e", "message", "110");
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "qlt_proqa_e", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","qlt/qlt_proqa_e.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResult.length > 0) {
        ...
        cfGetMessage("qlt_proqa_e", "message", "300");
        cfClearData("tbl_wrap", "oPaginate");
    }
}
//----------------------------------------------------------------
    ...
    switch(vEventId) {
        ...
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                cfAjaxAsync("GET","qlt_proqa_e",oData,"txt_cvcod");
                } else {
					cfSetValue("txt_cvnas","");
				}
            }
...
function onExcel() {	
	const vToday = cfGetToday();
	fnExcelReport('tbl_wrap_excel', '품질개선통보-'+ cfStringFormat(vToday, "D1"));
}
...
function cfGetAuth() {
	...
	cfAjaxSync("POST", "qlt_proqa_e", aData, "getAuth");
}
