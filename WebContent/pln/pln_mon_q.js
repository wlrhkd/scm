    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("pln_mon_q", "message" ,"110");
                return false;
            }
		...
		case "txt_itnbr":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("scm/pln_mon_q", "message" ,"110");
...
function pgLoadData(page) {
    ...
    if (vCvcod == "" || vCvcod == null) {
        cfGetMessage("scm/pln_mon_q", "message", "250");
        return false;
    }
    const oData = {
		ActGubun: "R",
    ...
    let oResponse = cfAjaxSync("POST", "scm/pln_mon_q", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/pln/pln_mon_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
        ...
        for (i = 0; i < oResult.length; i++) {
            ...
            gvOrgData.push({
				...
				"QTY_M1": aItem["QTY_M1"],  // m+1 월 계획량   
                "QTY_M2": aItem["QTY_M2"],	// m+2 월 계획량
            });
        }
        $("#tbl_wrap").show();
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/pln_mon_q", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/pln/pln_mon_q.txt",null,"table"); //테이블 정보 읽어오기
...
$(document).keydown(function (e) { 
    ...
    switch(vEventId) {
        ...
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                if (vCvcod != "") {
                    ...
                    cfAjaxAsync("GET","scm/pln_mon_q",oData,"txt_cvcod");                    
                } else{
					cfSetValue("txt_cvnas","");
				}
            }
		...
		case "txt_itnbr":
                    ...
                    cfAjaxAsync("GET","scm/pln_mon_q",iData,"txt_itnbr");              
                } else {
					cfSetValue("txt_itdsc","");
				}
            }
...
function cfGetAuth() {
	...
	cfAjaxSync("POST", "scm/pln_mon_q", aData, "getAuth");
}
