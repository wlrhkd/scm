...
//----------------------------------------------------------------------------------------------------------------------------
    ...
    cfMaxLength("txt_Yymd", "L");   //최대길이 설정 "S" 4자리, "M" 7자리, "L" 10자리

//----------------------------------------------------------------
// 콤보박스
//----------------------------------------------------------------
    cfGetHeadCombo("pln_week_q", "saupj", "sel_saupj"); //사업장 "sel_saupj" : id값
    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("scm/pln_week_q", "message" ,"110");
                return false;
            }
...
function pgLoadData(page) {
    ...
    const sYymd = cfGetValue("txt_fromDate").replace(/-/gi, "");
    ...
    if (vCvcod == "" || vCvcod == null) {
        cfGetMessage("scm/pln_week_q", "message", "250");
        return false;
    }
    const oData = {
		ActGubun: "R",
    ...
    let oResponse = cfAjaxSync("POST", "scm/pln_week_q", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/pln/pln_week_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
		...
		console.log("sYymd ="+sYymd.substr(0,4)+"-"+sYymd.substr(4,2));
		vHeader = vHeader.replace("D1", cfGetValue("txt_fromDate"));
		vHeader = vHeader.replace("D2", cfDateCalc(sYymd, 0));
		vHeader = vHeader.replace("D3", cfDateCalc(sYymd, 1));
		vHeader = vHeader.replace("D4", cfDateCalc(sYymd, 2));
		vHeader = vHeader.replace("D5", cfDateCalc(sYymd, 3));
		vHeader = vHeader.replace("D6", cfDateCalc(sYymd, 4));
		vHeader = vHeader.replace("D7", cfDateCalc(sYymd, 5));
		vListHTML = vHeader;
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/pln_week_q", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/pln/pln_week_q.txt",null,"table"); //테이블 정보 읽어오기
...
$(document).keydown(function (e) { 
    ...
    switch(vEventId) {
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                if (vCvcod != "") {
                    ...
                    cfAjaxAsync("GET","scm/pln_week_q",oData,"txt_cvcod");                    
                } else{
					cfSetValue("txt_cvnas","");
				}
            }
		...
		case "txt_itnbr":
                    ...
                    cfAjaxAsync("GET","scm/pln_week_q",oData,"txt_itnbr");              
                } else {
					cfSetValue("txt_itdsc","");
				}
            }
...
// 5.사용자 정의 함수
 $('#txt_fromDate').datepicker({
            ...
            onClose: function( selectedDate ) {    
                ...
                $("#txt_Yymd").datepicker( "option", "minDate", selectedDate );
            }                
        });
...
function cfGetAuth() {
	...
	cfAjaxSync("POST", "scm/pln_week_q", aData, "getAuth");
}
