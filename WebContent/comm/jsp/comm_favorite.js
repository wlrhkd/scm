...
let favgvChk = 0;
	...
	$(document).on("click", "#btn_search_fav" ,function(){
		search_menu();
    });
	$(document).on("dblclick", "#btn_search_fav" ,function(){
		cfSetValue("txt_top_search","");
...
$(document).on('click', '#favo', function() {
    ...
    for (let i = 0; i < oResponse.DATA.length; i++) {
        ...
        vRows = "<tr>";
        vRows += "<td class='favtd'><div class='img_left'><img src='../img/main/icon_arrow_dot.png'></div><span class='favsp'>" + aItem.SUB2_NAME + "</span><img src='../img/common/icon_gnb_on.png' class='favimg'></td>";
...
$(document).on('click', '#tbl_tbody_favorite_p2 td span', function () {
	...
	const link = "/" + url.replaceAll("aspx","jsp").replaceAll("nap","ord").replaceAll("soje","soj").replaceAll("qa","qlt");
	location.href = link;
//	alert(link);
});
...
$(document).mouseup(function (e){
  var LayerPopup = $(".favorite-layer");
  var state = $('.favorite-btn');
...
function search_menu() {
		...
		const vname = "%"+cfGetValue("txt_top_search")+"%";
