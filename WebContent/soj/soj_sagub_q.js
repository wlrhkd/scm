...
// 콤보박스
//----------------------------------------------------------------
    cfGetHeadCombo("soj_sagub_q", "saupj", "sel_saupj"); //사업장 "sel_saupj" : id값     
    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("soj_sagub_q", "message" ,"110");
                return false;
            }
...
function pgLoadData(page) {
    ...
    if (vCvcod == "" || vCvcod == null) {
        cfGetMessage("scm/soj_sagub_q", "message", "250");
        return false;
    }
    const oData = {
		ActGubun: "R",
    ...
    let oResponse = cfAjaxSync("POST", "scm/soj_sagub_q", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/soj/soj_sagub_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
		...
		//format
            for (let i = 0; i < vBody.rows.length; i++) {
                ...
                vBody.rows[i].cells[9].innerText = cfStringFormat(vBody.rows[i].cells[9].innerText,"J"); 
            }
        //페이징
        const oPageNav = cfDrawPageNav(nTotalRecords, nRecordPerPage, nPageNo);
        $("#oPaginate").html(oPageNav);
    } else {
        cfGetMessage("scm/soj_sagub_q", "message", "110");
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/soj_sagub_q", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/soj/soj_sagub_q.txt",null,"table"); //테이블 정보 읽어오기
    ...
    switch(vEventId) {
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                cfAjaxAsync("GET","scm/soj_sagub_q",oData,"txt_cvcod");                    
            } else {
				cfSetValue("txt_cvnas","");
		 	}
        }
...
function cfGetAuth() {
	...
	cfAjaxSync("POST", "scm/soj_sagub_q", aData, "getAuth");
}