...
// 콤보박스
//----------------------------------------------------------------
    cfGetHeadCombo("soj_subul_e", "saupj", "sel_saupj"); //사업장 "sel_saupj" : id값
    setInitValue();
    ...
    switch (name) {
        case "txt_cvcod":
            ...
            if (response.DATA.length == 0) {
                ...
                cfGetMessage("soj_subul_e", "message" ,"110");
                return false;
            }
...
function pgLoadData(page) {
    ...
    if (vCvcod == "" || vCvcod == null) {
        cfGetMessage("scm/soj_subul_e", "message", "250");
        return false;
    }
    const oData = {
		ActGubun: "R",
    ...
    let oResponse = cfAjaxSync("POST", "scm/soj_subul_e", oData, "startSelect");
    ...
    const vTable = cfAjaxSync("GET","scm/soj/soj_subul_e.txt",null,"table"); //테이블 정보 읽어오기
    ...
    if (oResponse.RecordCount > 0) {
		...
		vHeader = vHeader.replace("QTY_M1", cfMonthCalc(sYymm, 1));
		vHeader = vHeader.replace("QTY_M2", cfMonthCalc(sYymm, 2));
		vListHTML = vHeader;
        ...
        for (i = 0; i < oResult.length; i++) {
            ...
            gvOrgData.push({
				...
				"JUN_QTY": aItem["JUN_QTY"], // 전월이월       
				"IN_QTY": aItem["IN_QTY"],  // 입고                     
                "OUT_QTY": aItem["OUT_QTY"], // 출고  
                "JEGO_QTY": aItem["JEGO_QTY"], // 재고
                "CVNAS2": aItem["CVNAS2"], // 업체명
                "DANGA": aItem["DANGA"] // 단가
            });
        }
        $("#tbl_wrap").show();
        ...
        cfGetMessage("scm/soj_subul_e", "message", "110");
...
function pgLoadDataRs() {
    ...
    oResponse = cfAjaxSync("POST", "scm/soj_subul_e", sData, "startSelectS");
    if (!oResponse) {
        return false;
    }
    let vListHTML = "";
    ...
    const sTable = cfAjaxSync("GET","scm/soj/soj_subul_e.txt",null,"table"); //테이블 정보 읽어오기
        ...
        case "txt_cvcod":
            ...
            if (vEventCode == 13 || vEventCode == 9) {
                ...
                if (vCvcod != "") {
                    ...
                    cfAjaxAsync("GET","scm/soj_subul_e",oData,"txt_cvcod");                    
                } else {
					cfSetValue("txt_cvnas","");
				}
            }
...
// 5.사용자 정의 함수
 $('#txt_Yymm').datepicker({
            ...
            dateFormat: "Yyyymm",             // 날짜의 형식
            //changeMonth: true,                  // 월을 이동하기 위한 선택상자 표시여부
            //minDate: 0,                       // 선택할수있는 최소날짜, ( 0 : 오늘 이전 날짜 선택 불가)
            onClose: function( selectedDate ) {    
                // 시작일(fromDate) datepicker가 닫힐때
                // 종료일(toDate)의 선택할수있는 최소 날짜(minDate)를 선택한 시작일로 지정
                $("#txt_Yymm").datepicker( "option", "minDate", selectedDate );
            }                
        });
...
function cfGetAuth() {
	...
	cfAjaxSync("POST", "scm/soj_subul_e", aData, "getAuth");
}
